...
from bmmae.loops import loop_classification
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        choices=["IDH", "MGMT", "TERT", "project_id"],
    ...
    if wandb_logging:
        ...
        run = wandb.init(
            ...
            entity=args.entity,
            name=f"{args.task}_{'-'.join(args.modalities)}_{args.from_scratch}_froze={args.froze}",
            reinit=True,
            config=vars(args),
        )
    ...
    device = "cuda"
    # remove rows where biomarker is None
    dataframe = dataframe[~dataframe[args.task].isna()]
    labels = dataframe[args.task].unique()
    # perform cross validation
    total_ap = []
    total_auc = []
    kfold = StratifiedKFold(n_splits=5, shuffle=True, random_state=args.seed)
    for i, (train_index, test_index) in enumerate(kfold.split(dataframe, dataframe[args.task])):
        ...
        for i, path in enumerate(dataframe_train.MRI):
            train_files[i]["label"] = (
                0.0 if dataframe_train[dataframe_train.MRI == path][args.task].values[0] == labels[0] else 1.0
            )
        ...
        for i, path in enumerate(dataframe_val.MRI):
            val_files[i]["label"] = (
                0.0 if dataframe_val[dataframe_val.MRI == path][args.task].values[0] == labels[0] else 1.0
            )
        ...
        total_ap.append(metrics["val/ap"][-1])
        total_auc.append(metrics["val/roc_auc"][-1])

    total_ap = np.array(total_ap)
    total_auc = np.array(total_auc)
    logs = {
        "average_precision_mean": total_ap.mean(),
        "average_precision_std": total_ap.std(),
        "auc_mean": total_auc.mean(),
        "auc_std": total_auc.std(),
    }
    print(logs)
    if wandb_logging:
        wandb.log(logs)
